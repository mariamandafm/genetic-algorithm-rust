...
pub type Route = Vec<Point>;
    ...
    pub fn total_distance(route: &Route) -> f64 {
        route.windows(2).map(|w| distance(&w[0], &w[1])).sum()
    }
    ...
    // Seleção por torneio
    fn tournament_selection(rng: &mut ThreadRng, population: &[Route], k: usize) -> Route {
    ...
    pub fn crossover(rng: &mut ThreadRng, parent1: &Route, parent2: &Route) -> Route {
        ...
        let mut child = vec![[0.0, 0.0]; size]; 
        let mut genes_in_child = HashSet::with_capacity(size);
    ...
    // Mutação de uma rota
    fn mutate(rng: &mut ThreadRng, route: &mut Route, mutation_rate: f64) {
